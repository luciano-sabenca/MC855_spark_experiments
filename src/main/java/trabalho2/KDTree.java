...
class KDNode<T> implements Serializable {
    ...
    public KDNode<T> FindParent(double[] x0) {
    ...
    public KDNode Insert(double[] p, T t) {
        //x = new double[2];
...
public class KDTree<T> implements Serializable {
    ...
    public boolean add(double[] x, T t) {
        if (nList >= 2000000 - 1)
    ...
    public KDNode<T> find_nearest(double[] x) {
        if (Root == null)
            return null;

        checked_nodes = 0;
        KDNode<T> parent = Root.FindParent(x);
